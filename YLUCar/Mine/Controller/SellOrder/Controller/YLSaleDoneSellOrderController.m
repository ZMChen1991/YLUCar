//
//  YLSaleDoneSellOrderController.m
//  YLUCar
//
...
#define YLSaleDoneSaleOrderPath [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"SaleDoneSaleOrder.txt"]

@interface YLSaleDoneSellOrderController ()

@property (nonatomic, strong) NSMutableArray *saleOrders;
@property (nonatomic, strong) YLNoneView *noneView;
@end

@implementation YLSaleDoneSellOrderController
...
- (void)loadData {
    ...
    [param setValue:@"4" forKey:@"status"];
    __weak typeof(self) weakSelf = self;
    [YLRequest GET:urlString parameters:param success:^(id  _Nonnull responseObject) {
        if ([responseObject[@"code"] isEqualToNumber:[NSNumber numberWithInteger:200]]) {
            [weakSelf keyedArchiverObject:responseObject toFile:YLSaleDoneSaleOrderPath];
...
- (void)getLocalData {
    ...
    [self.saleOrders removeAllObjects];
    NSDictionary *dict = [NSKeyedUnarchiver unarchiveObjectWithFile:YLSaleDoneSaleOrderPath];
