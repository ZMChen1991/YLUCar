...
#define YLStayOnSaleOrderPath [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"StayOnSaleOrder.txt"]

@interface YLStayOnSellOrderController ()

@property (nonatomic, strong) NSMutableArray *saleOrders;
@property (nonatomic, strong) YLNoneView *noneView;
@end

@implementation YLStayOnSellOrderController
...
- (void)loadData {
    ...
    [YLRequest GET:urlString parameters:param success:^(id  _Nonnull responseObject) {
        if ([responseObject[@"code"] isEqualToNumber:[NSNumber numberWithInteger:200]]) {
            [weakSelf keyedArchiverObject:responseObject toFile:YLStayOnSaleOrderPath];
...
- (void)getLocalData {
    ...
    [self.saleOrders removeAllObjects];
    NSDictionary *dict = [NSKeyedUnarchiver unarchiveObjectWithFile:YLStayOnSaleOrderPath];
