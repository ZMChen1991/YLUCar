...
#define YLDealBuyOrderPath [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"DealBuyOrder.txt"]

@interface YLDealBuyOrderController ()
...
@implementation YLDealBuyOrderController
...
//        NSLog(@"param :%@ respronseObject:%@",param, responseObject);
        if ([responseObject[@"code"] isEqualToNumber:[NSNumber numberWithInteger:200]]) {
            [weakSelf keyedArchiverObject:responseObject toFile:YLDealBuyOrderPath];
...
- (void)getLocalData {
    ...
    [self.buyOrders removeAllObjects];
    NSDictionary *dict = [NSKeyedUnarchiver unarchiveObjectWithFile:YLDealBuyOrderPath];
