...
#define YLCancelBuyOrderPath [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"CancelBuyOrder.txt"]

@interface YLCancelBuyOrderController ()
...
@implementation YLCancelBuyOrderController
...
- (void)loadData {
    ...
    [param setValue:@"20" forKey:@"status"];
    [YLRequest GET:urlString parameters:param success:^(id  _Nonnull responseObject) {
//        NSLog(@"param :%@ respronseObject:%@",param, responseObject);
        if ([responseObject[@"code"] isEqualToNumber:[NSNumber numberWithInteger:200]]) {
            [weakSelf keyedArchiverObject:responseObject toFile:YLCancelBuyOrderPath];
...
- (void)getLocalData {
    ...
    [self.buyOrders removeAllObjects];
    NSDictionary *dict = [NSKeyedUnarchiver unarchiveObjectWithFile:YLCancelBuyOrderPath];
