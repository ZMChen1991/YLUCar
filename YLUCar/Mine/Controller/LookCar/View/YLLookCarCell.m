...
@implementation YLLookCarCell

+ (instancetype)cellWithTableView:(UITableView *)tableView {
    
    static NSString *ID = @"YLSubCell";
    YLLookCarCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[YLLookCarCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID];
...
- (void)setCellFrame:(YLLookCarCellFrame *)cellFrame {
    ...
    self.lookCarTime.frame = cellFrame.lookCarTimeF;
    ...
    if ([cellFrame.model.detail.status isEqualToString:@"3"]) {
        self.lookCarTime.text = [NSString stringWithFormat:@"看车时间:%@", cellFrame.model.appointTime];
    } else {
